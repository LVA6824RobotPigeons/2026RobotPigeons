...
public class ManualDriveCommand extends Command {
    ...
    // Delay after manual rotation input is released before heading-hold takes over.
    private static final Time kHeadingLockDelay = Seconds.of(0.25);
    ...
    public void seedFieldCentric() {
        // Reset lock state to prevent stale target headings after reseeding perspective.
        initialize();
    ...
    private void lockHeadingIfRotationStopped(ManualDriveInput input) {
        if (input.hasRotation()) {
            // Manual rotation always wins. Clear heading-hold target while stick is active.
    ...
    public void execute() {
        ...
        if (input.hasRotation()) {
            ...
            // Keep translating with heading lock if a lock target exists, otherwise stay in manual-rotation mode so lock can engage after kHeadingLockDelay.
            ...
            // Send an idle request once when sticks return to center.
            currentState = State.IDLING;
        }
        previousInput = input;
