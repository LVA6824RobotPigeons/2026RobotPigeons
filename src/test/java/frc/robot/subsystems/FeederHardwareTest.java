...
class FeederHardwareTest {
    ...
    void setup() throws Exception {
        feeder = new Feeder();
        motor = readMotorField(feeder, "motor");
    ...
    @Test
    void setFeed_usesVelocityControlMode() {
        feeder.set(Feeder.Speed.FEED);
        waitOneLoop();
        assertTrue(controlModeString(motor).contains("Velocity"));
    }

    @Test
    void setPercentOutput_usesVoltageControlMode() {
        feeder.setPercentOutput(0.5);
        waitOneLoop();
    ...
    void feedCommand_startsAndStopsAsExpected() {
        final Command command = feeder.feedCommand();
